...
import { X, Check, Sparkles, Monitor, Sun, Moon, Smartphone, Laptop } from 'lucide-react';
...
function PhoneMockup({ theme }: { theme: Theme }) {
  ...
  return (
    <div className="relative mx-auto shrink-0" style={{ width: 80, height: 148 }}>
  ...
  const colorModes: { id: ColorMode; label: string; icon: React.ReactNode }[] = [
    {
      id: 'system',
      label: 'System',
      icon: isMobile ? <Smartphone size={15} /> : <Laptop size={15} />,
    },
          ...
          <motion.div
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-white/10 overflow-hidden">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/10">
                <div>
                  <h2 className="text-lg font-extrabold text-gray-900 dark:text-white">Appearance</h2>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Customize your dashboard look and feel</p>
                </div>
              ...
              <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
                ...
                {/* Color mode */}
                <div>
                  <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">Color Mode</p>
                  <div className="grid grid-cols-3 gap-2">
                    {colorModes.map(({ id, label, icon }) => (
                      <button key={id} onClick={() => onColorModeChange(id)}
                        className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 text-sm font-bold transition-all cursor-pointer ${
                          colorMode === id
                          ...
                          <div className="flex items-center gap-3 p-3" style={{ background: theme.preview.bg }}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-1.5 mt-2.5">
                                {[theme.accent, theme.preview.header, theme.preview.bg, theme.preview.card].map((c, i) => (
                                  <div key={i} className="w-3 h-3 rounded-full border border-black/10 shadow-sm" style={{ background: c }} />
                                ))}
                              </div>
                            </div>
                          </div>
