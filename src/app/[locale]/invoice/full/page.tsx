...
export default function InvoiceFullPage() {
  ...
  const declaredValue = num(data?.breakdown?.declaredValue ?? data?.declaredValue ?? 0) || 0;
  ...
  const status: InvoiceStatus = (data?.status as InvoiceStatus) || (data?.paid ? "paid" : "pending");
  ...
  const companyAddress =
    safeStr(data?.company?.address) || "1199 E Calaveras Blvd, California, USA 90201";
  ...
  const statusBadge =
    status === "paid" ? "PAID" : status === "overdue" ? "OVERDUE" : status === "cancelled" ? "CANCELLED" : "PAYMENT PENDING";
                  ...
                  <div className="p-6 sm:p-8">
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
                      <div className="flex items-start gap-4 min-w-0">
                        <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-2xl bg-white/95 border border-white/60 overflow-hidden flex items-center justify-center shadow">
                          <Image
                            src="/logo.svg"
                        ...
                        <div className="min-w-0 text-gray-900">
                          <p className="text-sm sm:text-base font-extrabold">{companyName}</p>
            ...
            <div className="p-6 sm:p-8">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                ...
                <div className="rounded-2xl border border-gray-200 p-4">
                  ...
                  <p className="mt-2 text-sm text-gray-800 font-semibold">{fmtDate(data?.dates?.createdAt || null)}</p>
                  <p className="mt-1 text-xs text-gray-600">
                    Updated: <span className="font-semibold">{fmtDate(data?.dates?.updatedAt || null)}</span>
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-gray-200 p-4">
                  ...
                  <div className="mt-3 space-y-2 text-sm">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900 break-all">{shipmentId || "—"}</p>
                    </div>

                    <div>
                      <p className="text-xs text-gray-600">Tracking</p>
                      <p className="font-semibold text-gray-900 break-all">{trackingNumber || "—"}</p>
                    </div>

                    <div>
                      <p className="text-xs text-gray-600">Invoice number</p>
                      <p className="font-semibold text-gray-900 break-all">{invoiceNumber || "—"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-gray-200 p-4">
                  ...
                  <div className="mt-3 space-y-2 text-sm">
                    ...
                    <p className="text-xs text-gray-600">
                      Shipment status:{" "}
                      <span className="font-semibold text-gray-900">{safeStr(data?.shipment?.status) || "—"}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-gray-200 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{fmtMoney(declaredValue, currency)}</p>
                    </div>
                    <div>
              ...
              <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4 avoid-break">
                ...
                <div className="rounded-3xl border border-gray-200 bg-white shadow p-6">
                  ...
                  <div className="mt-5 rounded-2xl border border-gray-200 bg-white p-4">
                    ...
                    <p className="mt-1 text-sm font-extrabold text-gray-900">{paymentMethod ? paymentMethod : "NULL"}</p>
                ...
                <div className="rounded-3xl border border-gray-200 bg-white shadow p-6">
                  <div className="flex items-center gap-2">
                    {/* changed icon to match “previous style” (FileText) */}
                    <FileText className="w-5 h-5 text-blue-700" />
                    <h2 className="text-lg font-extrabold text-gray-900">Charges summary</h2>
                  </div>

                  <p className="mt-1 text-sm text-gray-600">Charges are calculated from your declared value.</p>
                  ...
                  <div className="mt-5 space-y-2 text-sm">
                    ...
                    <Row label={`Discount (${fmtPercent(rates.discount)})`} value={fmtMoney(calc.discount, currency)} />

                    <div className="pt-3 mt-3 border-t border-gray-200 flex items-center justify-between">
                      <span className="text-gray-900 font-semibold">Subtotal</span>
                      <span className="text-gray-900 font-semibold">{fmtMoney(calc.subtotal, currency)}</span>
                    </div>
