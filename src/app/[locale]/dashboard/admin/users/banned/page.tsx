...
import {
  RefreshCw, Ban, Loader2, CheckCircle2, XCircle, RotateCcw,
} from "lucide-react";
...
export default function BannedUsersPage() {
  const [items, setItems] = useState<BU[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [busyId, setBusyId] = useState<string | null>(null);
  const [confirmAction, setConfirmAction] = useState<{ id: string; type: "unban" } | null>(null);
  ...
  const load = async (silent = false) => {
    ...
    try {
      const res = await fetch("/api/admin/users/unban", { cache: "no-store" });
  ...
  const unban = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/users/unban`, {
        ...
        body: JSON.stringify({ email: items.find(u => u.id === id)?.email }),
      });
      if (!res.ok) {
        const t = await res.text().catch(() => "");
        showMsg(`Unban failed: ${t.slice(0, 100)}`, "error");
        return;
      }
      setItems(prev => prev.filter(x => x.id !== id));
      showMsg("User unbanned successfully.");
    } catch {
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/80">
                    {["#", "User", "Email", "Reason", "Banned At", "Actions"].map(h => (
                ...
                <tbody className="divide-y divide-gray-50">
                  {items.map((u, idx) => (
                    <tr key={u.id} className="group hover:bg-slate-50/80 transition">
                      ...
                      <td className="py-4 px-4 whitespace-nowrap text-sm text-gray-600">{u.email || "—"}</td>
                      <td className="py-4 px-4 text-xs text-gray-500 max-w-[200px] truncate">{u.reason || "—"}</td>
                      <td className="py-4 px-4 whitespace-nowrap text-xs text-gray-400 font-medium">{fmtDate(u.bannedAt)}</td>
                      <td className="py-4 px-4 whitespace-nowrap text-right">
                        <button type="button"
                          onClick={() => setConfirmAction({ id: u.id, type: "unban" })}
      ...
      {confirmAction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl border border-gray-200 p-6">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4 bg-emerald-100">
              <RotateCcw className="w-6 h-6 text-emerald-600" />
            </div>
            <h3 className="text-xl font-extrabold text-gray-900">Unban this user?</h3>
            {confirmUser && (
            ...
            <p className="mt-3 text-sm text-gray-500 leading-relaxed">
              This will remove the email from the banned list and restore the user's account. They will receive an email notification.
            </p>
            <div className="mt-6 flex items-center justify-end gap-3">
              ...
              <button type="button" onClick={() => unban(confirmAction.id)}
                disabled={busyId === confirmAction.id}
                className="cursor-pointer px-4 py-2.5 rounded-xl text-white text-sm font-semibold transition disabled:opacity-60 flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700">
