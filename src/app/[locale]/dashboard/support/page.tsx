...
import {
  MessageCircle, FileText, Plus, ChevronRight, Loader2,
  AlertCircle, Search, ChevronLeft, ChevronRight as ChevronRightIcon,
  X, Send,
} from "lucide-react";
import { THEMES, type ThemeId } from "@/components/AppearancePanel";
import LiveChatWidget from "@/components/support/LiveChatWidget";
import {
  SUPPORT_CATEGORIES, categoryLabel, categoryEmoji,
...
export default function UserSupportPage() {
  ...
  // ─── Tab state ─────────────────────────────────────────────
  // Optional ?tab=tickets|chat — defaults to chat
  const initialTab = (sp.get("tab") === "tickets") ? "tickets" : "chat";
  ...
  // Resolve the user's id (for chat channel)
    ...
    <div className="max-w-4xl mx-auto pb-12 space-y-5">
      ...
      <div>
        ...
        <p className={`mt-1 text-sm ${headerSubCls}`}>
          Chat with our team or open a ticket for tracked issues.
        </p>
      </div>
      ...
      <div className="flex gap-1 rounded-xl bg-gray-100 dark:bg-white/5 p-1 max-w-md">
        <button
          ...
          style={tab === "chat" ? { background: accentGradient } : {}}>
...
function TicketsTab({
  accentSolid, accentGradient, locale,
}: {
  accentSolid: string; accentGradient: string; locale: string;
}) {
  const router = useRouter();
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      r = r.filter(t =>
        t.ticketNumber.toLowerCase().includes(v) ||
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/10">
              {paged.map(t => {
                ...
                return (
                  <Link
                    key={t._id}
                    href={`/${locale}/dashboard/support/${t._id}`}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {t.unreadByUser > 0 && (
                            <span className="inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold text-white rounded-full"
      ...
      {showNewTicket && (
        <NewTicketModal
          ...
          onCreated={(t) => {
            ...
            router.push(`/${locale}/dashboard/support/${t._id}`);
          }}
        />
      )}
    </div>
      ...
      <div className="relative w-[92%] max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-white/10 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                  ...
                  <button key={c.id} type="button" onClick={() => setCategory(c.id)}
                    className={`px-3 py-2.5 rounded-xl text-xs font-bold transition cursor-pointer text-left border-2 flex items-center gap-2 ${
                      isActive
                        ...
                        : "bg-gray-50 dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-200 hover:border-gray-300"
                    }`}
                    style={isActive ? { background: accentSolid } : {}}>
          ...
          <div>
            ...
            <input
              value={shipmentRef}
              onChange={e => setShipmentRef(e.target.value.toUpperCase())}
