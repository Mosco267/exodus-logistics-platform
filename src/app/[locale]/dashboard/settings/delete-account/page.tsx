...
export default function DeleteAccountPage() {
  ...
  const handlePasswordAuth = async () => {
   if (!password) { setAuthError(t('DeleteAccount.errPasswordRequired')); return; }
    ...
    try {
      ...
      if (!res.ok) { setAuthError(data.error || t('DeleteAccount.errIncorrectPassword')); return; }
      setStep('confirm');
    } catch { setAuthError(t('DeleteAccount.errGeneric')); }
    finally { setAuthLoading(false); }
  };

  const handleDelete = async () => {
    if (deleteConfirmText !== 'DELETE') { setDeleteError(t('DeleteAccount.errTypeDelete')); return; }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setDeleteError(data.error || t('DeleteAccount.errFailed')); return; }
      setStep('done');
      setTimeout(() => signOut({ callbackUrl: `/${locale}/sign-in` }), 2500);
    } catch { setDeleteError(t('DeleteAccount.errGeneric')); }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-gray-900 dark:text-white"
            style={isMidnight ? { color: '#ffffff' } : {}}>{t('DeleteAccount.title')}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{t('DeleteAccount.subtitle')}</p>
        </div>
      </div>
  ...
  <div>
    <p className="text-sm font-bold text-amber-700 dark:text-amber-400">{t('DeleteAccount.restoreTitle')}</p>
    <p className="text-xs text-amber-600/80 dark:text-amber-400/70 mt-1 leading-relaxed">
      {t('DeleteAccount.restoreDesc', {
        ...
        {step === 'auth' && (
          <div className="space-y-4">
            <p className="text-sm font-bold text-gray-700 dark:text-gray-300">{t('DeleteAccount.confirmIdentity')}</p>
            <div>
              <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 block uppercase tracking-wide">{t('DeleteAccount.password')}</label>
            ...
            <button onClick={handlePasswordAuth} disabled={authLoading || !password}
              ...
              {authLoading ? t('DeleteAccount.verifying') : t('DeleteAccount.continue')}
            </button>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 block">
                {t('DeleteAccount.reasonLabel')} <span className="font-normal">{t('DeleteAccount.optional')}</span>
              </label>
              <textarea value={deleteReason} onChange={e => setDeleteReason(e.target.value)}
                placeholder={t('DeleteAccount.reasonPlaceholder')} rows={2}
                className={inputClass + ' resize-none'} style={{ fontSize: '16px' }} />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 block">
                {t('DeleteAccount.typeToConfirm', {
                  strong: (chunks: any) => <strong className="text-red-600">{chunks}</strong>,
                })}
              </label>
            ...
            <div className="flex gap-2.5">
              ...
              <button onClick={handleDelete} disabled={deleting || deleteConfirmText !== 'DELETE'}
                ...
                {deleting ? t('DeleteAccount.deleting') : t('DeleteAccount.deleteAccount')}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 'done' && (
          <div className="text-center space-y-4 py-4">
            ...
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('DeleteAccount.doneTitle')}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{t('DeleteAccount.doneDesc')}</p>
          </div>
        )}
      </div>
    </div>
