...
const toggleDark = () => {
  ...
  {/* Mobile menu button */}
  <button className="md:hidden p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 transition cursor-pointer text-gray-600 dark:text-gray-300 shrink-0"
  ...
  <Link href={`/${locale}/dashboard`} className="md:hidden shrink-0">
    <img src={darkMode ? '/logo-dark.svg' : '/logo-light.svg'} alt="Exodus" className="h-7 w-auto max-w-[110px]" />
  </Link>

  {/* Logo — desktop only */}
  <Link href={`/${locale}/dashboard`} className="hidden md:block shrink-0">
    <img src={darkMode ? '/logo-dark.svg' : '/logo-light.svg'} alt="Exodus" className="h-9 w-auto" />
  </Link>
