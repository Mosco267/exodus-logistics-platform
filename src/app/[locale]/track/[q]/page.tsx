                                      ...
                                      {(ev.entries || []).map((en, j) => {
                                        ...
                                        return (
                                          <div key={`entry-${j}`} className="relative pl-1">
                                            {!isLastEntry && (
  <div className="absolute left-[-7px] top-[14px] bottom-[-20px] w-[2px] rounded-full" style={{ background: `linear-gradient(to bottom, ${entryDotBg} 0%, ${safeColor(ev.entries?.[ev.entries.length - 1]?.color) || "#d1d5db"} 100%)` }} />
)}
