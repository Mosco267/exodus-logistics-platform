...
// starts fading after 100px, fully faded by 400px

  // Top-level state for flipped cards
...
const [animateWord, setAnimateWord] = useState(true); // true: fade-in, false: fade-out

useEffect(() => {
  const interval = setInterval(() => {
    setAnimateWord(false); // start fading out current word
    setTimeout(() => {
      setHeroWordIndex((prev) => (prev + 1) % heroWords.length);
      setAnimateWord(true); // fade in next word
    }, 800); // reduce wait time for faster overlap
  }, 8000); // total per word still 8s
...
// Start fading from bottom and fade completely at 40% scroll down the hero
  const fadeStart = heroOffset; // top of hero
  const fadeEnd = heroOffset + heroHeight * 1.8; // 40% down

  const heroOpacity = useTransform(scrollY, [fadeStart, fadeEnd], [1, 0]);
  const heroY = useTransform(scrollY, [fadeStart, fadeEnd], [40, 0]); // move up while fading

const controls = useAnimation();
const isInView = useInView(heroRef, { amount: 0.5 }); // 50% visibility
  ...
  const handleTyping = () => {
    if (!isDeleting) {
      ...
      if (displayedText === currentWord) {
        setTimeout(() => setIsDeleting(true), 2000); // pause 2s before deleting
      }
    } else {
      // Deleting
      setDisplayedText(currentWord.substring(0, displayedText.length - 1));
...
>
  ...
  <motion.span
    key={heroWordIndex}
    initial={{ opacity: 0, x: 50 }}       // entering from right
    animate={{ opacity: animateWord ? 1 : 0, x: 0 }}
    exit={{ opacity: 0, x: -50 }}         // exiting to left
...
<AnimatePresence mode="wait">
  <motion.p
    ...
    initial={{ opacity: 0, x: -50 }}      // enters from left
    animate={{ opacity: animateWord ? 1 : 0, x: 0 }}
    exit={{ opacity: 0, x: 50 }}          // exits to right
    transition={{ duration: 1, ease: "easeInOut" }}
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    >
      ...
      <p className="text-lg text-gray-600 max-w-2xl mx-auto">
        We combine cutting-edge technology with decades of logistics expertise to ensure your shipments arrive safely, on-time, and within budget.
      </p>
    </motion.div>
         ...
         className="relative w-full h-64 cursor-pointer"
          ...
          onClick={() =>
            setFlippedCards(prev => ({
              // flip current card, reset all others
...
">
  ...
  Ship with low cost solutions designed to protect your business and your customers. At Exodus Logistics, we prioritize safety, reliability, and complete transparency in every delivery.
</p>

<p className="text-lg max-w-2xl mx-auto mb-6 drop-shadow-sm">
  Gain confidence knowing your cargo is handled by experienced professionals, fully insured, carefully tracked, and delivered on time with precision and care.
</p>
            <motion.button
